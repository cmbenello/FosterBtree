...
use crate::prelude::{ContainerKey, FrameReadGuard, MemPool, Page, PageFrameKey, PageId};
...
impl<T: MemPool> GensortStore<T> {
    ...
    pub fn get_record(&self, index: usize) -> Option<Vec<u8>> {
        ...
        let mut current_page_id = self
            .mem_pool
            .get_page_for_read(self.root_key)
            .unwrap()
            .next_page()?;
        ...
        for _ in 0..page_number {
            let page = self
                .mem_pool
                .get_page_for_read(PageFrameKey::new(self.c_key, current_page_id))
                .unwrap();
...
mod tests {
    ...
    fn test_large_dataset_sampling() {
        ...
        for i in (0..num_records).step_by(100) {
            ...
            assert_eq!(
                actual_value,
                &expected_value[..RECORD_VALUE_SIZE],
        ...
        for record in store.scan() {
            ...
            assert_eq!(
                record_num, count,
                "Sequential scan: record at position {} has key {}",
                count, record_num
            );
